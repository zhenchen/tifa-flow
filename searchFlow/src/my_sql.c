...
void MySQL::addRec(struct in_addr sip,struct in_addr dip,unsigned short sport,unsigned short dport, int protocol, double time_start, double time_end, int fileno, int offset, int len)
{
	if( time_oldest < 1e-6 )time_oldest = time_start;
	if( time_part_oldest < 1e-6)time_part_oldest = time_end;
	if( time_end > time_latest )time_latest = time_end;
	...
	if(fp){
		...
		fprintf(fp,"%d,%f,%f,",protocol,time_start,time_end);
		fprintf(fp,"%d,%d,%d\n",fileno,offset,len);
		tot_rows++;
		tmp_rows++;
...
void MySQL::dump(char * dst, char * src)
{
	...
	sprintf(cmd,"./ardea -d %s -m 'sip1:unsigned byte,sip2:unsigned byte,sip3:unsigned byte,sip4:unsigned byte \
			...
			,proto:int,time_start:double,time_end:double,pcapfile:int,offset:int,len:int' -t %s >>storelog 2>&1\0",dst,src);
	system(cmd);
}
