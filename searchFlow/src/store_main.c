...
void pcap_callback(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) 
{
	...
	if(_global.conns)_global.conns->addpkt(sip,dip,sport,dport,protocol,time,header,packet);
}
...
void init()
{
	...
	_global.pkts_size = 0;
	_global.conns = new Connections(_run_settings.indexpart_outfile);

	system("rm -rf index/*");
	system("mkdir index/index");
	system("mkdir index/tmp");

	system("rm -rf traces/day/*");
}
