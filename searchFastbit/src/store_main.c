...
void parseOptions(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "hf:o:F:i:p:")) != EOF) {
		switch (opt) {
			...
			case 'i':
				...
				break;
			case 'p':
				_run_settings.indexpart_outfile.assign(optarg);
				break;
		}
	}
	...
	if( _run_settings.indexpart_outfile.empty() ){
		printf("No index parts output file assigned!\nPlease use -h to see help info.\n");
		exit(-1);
	}
}
...
void pcap_callback(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) 
{
	...
	if(_global._sql)_global._sql->addRow(sip,dip,sport,dport,protocol,time,header->caplen);
}
...
void runPcapBatchMode()
{
	...
	for(int i = 0; i < files.size() ; i++)
	{
		...
		if(!_global._pcap_h){
			...
			if(_global._sql)_global._sql->setNowFile(i+1);
...
void doExit()
{
	if( _global._pcap_h) pcap_close(_global._pcap_h);
	if( _global._sql) delete(_global._sql);
}	
...
void init()
{
	...
	_global.pkts_size = 0;
	_global._sql = new MySQL(_run_settings.indexpart_outfile);
